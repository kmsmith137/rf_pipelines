...
def make_random_polynomial_detrender(nfreq_ds, nds):
    ...
    if (rand.randint(0,2)) and (nfreq_ds >= 16):
        ...
        nt_chunk = 8 * max(rand.randint(-5,5), 0)
    else:
        axis = 'AXIS_TIME'
...
def make_random_transform_chain(nfreq_ds, nds):
    ret = [ make_random_polynomial_detrender(nfreq_ds, nds),
            make_random_intensity_clipper(nfreq_ds, nds) ]

    if nfreq_ds >= 32:
        ret.append(make_random_spline_detrender(nfreq_ds, nds))

    if (nfreq_ds % 8 == 0) and (nfreq_ds >= 32):
...
for iter in xrange(100):
    s = initial_stream()
    tj = make_random_transform_chain(s.nfreq, 1)
